...
def inject_custom_css():
    ...
    :root {
        ...
        --shadow-hover: 0 10px 15px -3px rgba(0,0,0,0.4), 0 4px 6px -4px rgba(0,0,0,0.3);
        --radius: 12px;
        --radius-sm: 8px;
        --radius-lg: 16px;
...
def signal_card_html(symbol, signal, price, change_pct, strength, reasons, signal_type="bullish"):
    ...
    reasons_html = "".join([f'<div style="color:#94a3b8;font-size:0.82rem;padding:2px 0;">â€¢ {r}</div>' for r in reasons[:4]])
    ...
    <div class="signal-card {signal_type}">
        <div style="display:flex;justify-content:space-between;align-items:center;">
            ...
            <div style="text-align:right;">
                <div style="font-family:'JetBrains Mono';font-weight:700;font-size:1.1rem;">â‚¹{price:,.2f}</div>
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        <div style="text-align:center;padding:15px 0;">
            <div class="main-title">ðŸ“Š AI Scanner</div>
...
def page_dashboard():
    ...
    <div class="main-header">
        <div class="main-title">ðŸ  Market Dashboard</div>
    ...
    with col_chart:
        st.markdown("#### ðŸ“ˆ NIFTY 50 Chart")
...
def page_scanner():
    ...
    st.markdown("""
    <div class="main-header">
        <div class="main-title">ðŸ” Stock Scanner</div>
    ...
    # Run Scanner
    if st.button("ðŸš€ Run Scanner", type="primary", use_container_width=True):
        with st.spinner("ðŸ“¡ Fetching data & running scanners..."):
            ...
            if all_results:
                ...
                c2.metric("ðŸŸ¢ Bullish", len(bullish))
                c3.metric("ðŸ”´ Bearish", len(bearish))
                c4.metric("Avg Strength", f"{np.mean([r.strength for r in all_results]):.0%}")
                
                st.markdown("---")
                
                # Tabs for results
                tab_all, tab_bullish, tab_bearish = st.tabs(["ðŸ“Š All Signals", "ðŸŸ¢ Bullish", "ðŸ”´ Bearish"])
...
def page_stock_analysis():
    ...
    st.markdown("""
    <div class="main-header">
        <div class="main-title">ðŸ“ˆ Stock Analysis</div>
    ...
    if symbol:
        ...
        c1.metric("Price", f"â‚¹{latest['close']:,.2f}", f"{change_pct:+.2f}%")
        c2.metric("Open", f"â‚¹{latest['open']:,.2f}")
        c3.metric("High", f"â‚¹{latest['high']:,.2f}")
        c4.metric("Low", f"â‚¹{latest['low']:,.2f}")
        ...
        tab_chart, tab_indicators, tab_volume, tab_fundamentals = st.tabs([
            "ðŸ“ˆ Price Chart", "ðŸ“Š Indicators", "ðŸ“¦ Volume", "ðŸ’° Fundamentals"
        ])
        ...
        with tab_fundamentals:
            try:
                ...
                if fund:
                    ...
                    c3.metric("Market Cap", f"â‚¹{fund.market_cap/1e7:,.0f} Cr")
                    ...
                    c7.metric("EPS", f"â‚¹{fund.eps:.2f}" if fund.eps else "N/A")
                    c8.metric("52W Range", f"â‚¹{fund.week_52_low:.0f} - â‚¹{fund.week_52_high:.0f}")
                    
                    if fund.description:
                        with st.expander("ðŸ“ Company Description"):
...
def page_ai_predictions():
    ...
    st.markdown("""
    <div class="main-header">
        <div class="main-title">ðŸ¤– AI Predictions</div>
            ...
            try:
                ...
                with c2:
                    ...
                    <div style="background:var(--bg-card);border-radius:12px;padding:20px;border:1px solid var(--border-color);">
                        <h4 style="color:var(--text-secondary);margin-bottom:15px;">ðŸ“Š Model Votes</h4>
...
def page_heatmap():
    ...
    st.markdown("""
    <div class="main-header">
        <div class="main-title">ðŸ“Š Market Heatmap</div>
...
def page_watchlist():
    ...
    st.markdown("""
    <div class="main-header">
        <div class="main-title">ðŸ“‹ Watchlist</div>
    ...
    if st.session_state.watchlist:
        ...
        for sym in st.session_state.watchlist:
            if sym in data and not data[sym].empty:
                ...
                col1.markdown(f"**{sym}**")
                col2.markdown(f"<span style='font-family:JetBrains Mono;font-weight:700;'>â‚¹{latest['close']:,.2f}</span>", unsafe_allow_html=True)
                ...
                if col5.button("âŒ", key=f"remove_{sym}"):
...
def page_portfolio():
    ...
    st.markdown("""
    <div class="main-header">
        <div class="main-title">ðŸ’¼ Portfolio</div>
    ...
    with st.expander("âž• Add Trade"):
        ...
        p_price = c2.number_input("Buy Price (â‚¹)", min_value=0.01, value=100.0)
    ...
    if st.session_state.portfolio:
            ...
            <div class="signal-card {'bullish' if pnl > 0 else 'bearish'}">
                <div style="display:flex;justify-content:space-between;">
                    <div>
                        ...
                        <span style="color:var(--text-muted);margin-left:10px;">{entry['qty']} shares @ â‚¹{entry['buy_price']:.2f}</span>
                    </div>
                    <div style="text-align:right;">
                        <div style="font-family:'JetBrains Mono';font-weight:700;">â‚¹{current_price:,.2f}</div>
                        <div style="color:{color};font-family:'JetBrains Mono';font-weight:600;">
                            {'+' if pnl > 0 else ''}â‚¹{pnl:,.2f} ({pnl_pct:+.2f}%)
                        </div>
                    </div>
                </div>
            </div>
        ...
        c1.metric("Total Investment", f"â‚¹{total_investment:,.2f}")
        c2.metric("Current Value", f"â‚¹{total_current:,.2f}")
...
def page_settings():
    ...
    st.markdown("""
    <div class="main-header">
        <div class="main-title">âš™ï¸ Settings</div>
        <div class="main-subtitle">Configure scanner preferences</div>
    </div>
    """, unsafe_allow_html=True)
    
    tab1, tab2, tab3 = st.tabs(["ðŸ“Š Scanner", "ðŸ”” Alerts", "ðŸ—„ï¸ Database"])
